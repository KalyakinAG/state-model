...
Функция Параметр(ПутьКДанным, Имя = "", ПроверкаЗаполнения = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ЭлементФормы) И НЕ ЗначениеЗаполнено(Свойство) Тогда
		...
		ПараметрЭлемента = РаботаСМодельюСостояния.СоздатьПараметрЭлемента(Схема, ЭлементФормы, Параметр);
...
Функция Использование(ИмяПараметра = "") Экспорт
	...
	РаботаСМодельюСостояния.СоздатьСвязь(Схема, ПараметрСостояния, ПараметрИспользование, "Использование");
...
Функция Свойство(ИмяСвойства, ИмяПараметра = "") Экспорт
	...
	ИмяПараметраСвойства = ?(ЗначениеЗаполнено(ИмяПараметра), ИмяПараметра, ОбщийКлиентСервер.ИмяПоУникальномуИдентификатору());
	ПараметрСостояния = РаботаСМодельюСостояния.ПараметрСостояния(Схема, ИмяПараметраСвойства);
...
Функция ПрименитьМодель() Экспорт
	...
	Если ТипЗнч(Контекст) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		Попытка
			РаботаСМодельюСостояния.УстановитьДействие(Контекст, "ПриЧтенииНаСервере", "ПриЧтенииВМоделиОбъектаНаСервере");
			РаботаСМодельюСостояния.УстановитьДействие(Контекст, "ПослеЗаписиНаСервере", "ПослеЗаписиВМоделиОбъектаНаСервере");
		Исключение
		КонецПопытки;
	Иначе
		РаботаСМодельюСостояния.ПрименитьМодельОбъекта(Схема);
