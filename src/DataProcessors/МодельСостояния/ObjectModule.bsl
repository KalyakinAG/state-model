...
//  СвязиПараметров[]
//
Функция СоздатьСвязь(ПараметрСостояния, Параметр, ПутьКДанным, Значение = Неопределено)
...
Функция ПолучитьПараметрСостояния(ПутьКДанным = "", Имя = "", Контекстный = Неопределено)
	...
	Если ЭтоРеквизитИлиСвойство Тогда
		...
		Если СоставПути.Количество() = 1 Тогда
			...
			Если (Ложь
				...
				ИЛИ СтрНайти(типЗначения, "ТабличнаяЧасть.") > 0
			)
			Тогда
...
Функция Параметр(ПутьКДанным, Имя = "") Экспорт
	ИмяПараметра = ОбщийКлиентСервер.ЕстьПустоеЗначение(Имя, ПутьКДанным);
	...
	Если ЗначениеЗаполнено(ЭлементФормы) И НЕ ЗначениеЗаполнено(Свойство) Тогда
		...
		Возврат ЭтотОбъект;
	КонецЕсли;
	Связь = СоздатьСвязь(ПараметрСостояния, Параметр, ПутьКДанным);
	Возврат ЭтотОбъект;
КонецФункции
...
Функция ЭлементФормы(ЭлементИлиИмя = "ЭтаФорма") Экспорт
	...
	Если ЗначениеЗаполнено(ПутьКДанным) Тогда
		ЭлементФормы.Параметр = ПолучитьПараметрСостояния(ПутьКДанным);
	КонецЕсли;
...
Функция Использование(ИмяПараметра = "") Экспорт
	ИмяПараметраСвойства = ?(ЗначениеЗаполнено(ИмяПараметра), ИмяПараметра, ОбщийКлиентСервер.ИмяПоУникальномуИдентификатору());
	ПараметрИспользование = ПолучитьПараметрСостояния(ИмяПараметраСвойства);
