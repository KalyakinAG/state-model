...
Функция Выражение(ТекстВыражения = "*") Экспорт
	Если ТекстВыражения = "*" Тогда
		ПараметрСостояния.Выражение = СтрШаблон("%1.ЗначениеПараметра%2(Параметры, Значение, СтандартнаяОбработка, Отказ)", Схема.Модуль, ПараметрСостояния.Имя);
	Иначе
...
Функция ПараметрПризнака(ИмяПризнака, ИмяПараметра = "") Экспорт
	...
	РаботаСМодельюСостояния.СоздатьСвязь(Схема, ПараметрСостояния, ПараметрИспользование, ИмяПризнака);// Фиктивная связь с параметром признака Использование
...
Функция ПроверкаЗаполнения(Значение = Истина) Экспорт
	Если ЗначениеЗаполнено(Связь) Тогда
		Связь.ПроверкаЗаполнения = Значение;
		Возврат ЭтотОбъект;		
	КонецЕсли;
	Если ЗначениеЗаполнено(ПараметрЭлемента) Тогда
		ПараметрЭлемента.ПроверкаЗаполнения = Значение;
		Возврат ЭтотОбъект;		
	КонецЕсли;
	ПараметрСостояния.ПроверкаЗаполнения = Значение;
	Возврат ЭтотОбъект;
КонецФункции

Функция НеПроверкаЗаполнения() Экспорт
