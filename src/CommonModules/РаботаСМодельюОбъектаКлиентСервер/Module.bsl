...
Функция Заполнить(Контекст, Шаблон) Экспорт
	...
	Для Каждого ЭлементШаблона Из Шаблон Цикл
		...
		Если ЗначениеЗаполнено(Параметр.ПараметрТаблица) Тогда
			...
			МассивСтрок = ЭлементШаблона.Значение;
			Для Каждого ИндексСтроки Из МассивСтрок.ВГраница() Цикл
				ЭлементыСтроки = МассивСтрок[ИндексСтроки];
				Если ЭлементыСтроки = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				СтрокаТаблицы = Таблица[ИндексСтроки];
				Если МодельОбъекта.ЭтоКонтекстФормы Тогда
					ИдентификаторСтроки = Таблица[ИндексСтроки].ПолучитьИдентификатор();
				Иначе
					ИдентификаторСтроки = ИндексСтроки;
				КонецЕсли;
				Для Каждого ЭлементСтроки Из ЭлементыСтроки Цикл
					Если НЕ ОбщийКлиентСервер.УстановитьЗначение(СтрокаТаблицы[ЭлементСтроки.Ключ], ЭлементСтроки.Значение) Тогда
						Продолжить;
					КонецЕсли;
					ИмяПараметра = ЭлементШаблона.Ключ + "." + ЭлементСтроки.Ключ;
					ПриИзмененииПараметра(МодельОбъекта, ИмяПараметра, ИдентификаторСтроки);
				КонецЦикла;
			КонецЦикла;
			ПриИзмененииПараметра(МодельОбъекта, Параметр.ПараметрТаблица.Имя);
			Продолжить;
		КонецЕсли;
