...
Процедура ПередНачаломИзмененияСтрокиТаблицыФормы(Форма, ТаблицаФормы, Отказ) Экспорт
	...
	Для Каждого ЭлементКолонки Из ПараметрТаблицы.Колонки Цикл
...
Процедура ПередОкончаниемРедактированияСтрокиТаблицыФормы(Форма, ТаблицаФормы, НоваяСтрока, ОтменаРедактирования, Отказ) Экспорт
	...
	КонецЕсли;
	Таблица = РаботаСМодельюОбъектаКлиентСервер.ПолучитьЗначениеПараметра(МодельОбъекта, ПараметрТаблицы);
	Колонки = ПараметрТаблицы.Колонки;
	...
	Для каждого ЭлементКлюча Из ЗначенияКлюча Цикл
		Если НЕ ЗначениеЗаполнено(ЭлементКлюча.Значение) Тогда
			ОбязательныеПоля.Добавить(Колонки[ЭлементКлюча.Ключ]);
		КонецЕсли;
	КонецЦикла;
	...
	Если НайденныеСтроки.Количество() > 1 Тогда
		...
		Для каждого ЭлементКлюча Из ЗначенияКлюча Цикл
			ПредставленияКолонок.Добавить(Колонки[ЭлементКлюча.Ключ]);									
		КонецЦикла;
