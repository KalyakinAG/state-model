...
Функция СоздатьПараметр(Схема, ИмяПараметра, Вычисляемый = Ложь) Экспорт
	...
	НовыйПараметрСостояния.Вставить("Колонки", Неопределено);
	...
	Если ЭтоРеквизитИлиСвойство Тогда
		...
		Если СоставПути.Количество() = 1 Тогда
			...
			Если ТипЗначения = Тип("ТаблицаЗначений") Тогда
				ПараметрСостояния.Колонки = КолонкиТаблицы(Значение.Колонки);
				ПараметрСостояния.ЭтоТаблица = Истина;
			ИначеЕсли ТипЗначения = Тип("ДанныеФормыКоллекция") Тогда
				ПараметрСостояния.Колонки = КолонкиТаблицы(Значение.Выгрузить(Новый Массив).Колонки, "ИсходныйНомерСтроки, НомерСтроки");
				ПараметрСостояния.ЭтоТаблица = Истина;
			ИначеЕсли СтрНайти(ТипЗначения, "ТабличнаяЧасть.") > 0 Тогда
				ПараметрСостояния.Колонки = КолонкиТаблицы(Значение.Выгрузить(Новый Массив).Колонки, "НомерСтроки");
				ПараметрСостояния.ЭтоТаблица = Истина;
			КонецЕсли;
		Иначе
...
Функция КолонкиТаблицы(Колонки, ИсключитьКолонки = "")
	...
	Для Каждого Колонка Из Колонки Цикл
		Если КолонкиИсключения.Найти(Колонка.Имя) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ОписанияКолонок.Вставить(Колонка.Имя, ?(ПустаяСтрока(Колонка.Заголовок), Колонка.Имя, Колонка.Заголовок));		
	КонецЦикла;
